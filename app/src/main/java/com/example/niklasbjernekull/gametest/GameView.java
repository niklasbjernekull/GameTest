...
class GameView extends SurfaceView implements Runnable {
    ...
    public GameView(Context context) {
        ...
        skull = new Skull(this.getResources(), 0, 0);
        skull.scale(50,50);

        tree = new Tree(this.getResources(), 720, 50);
        tree.scale(120,200);

        flower = new Flower(this.getResources(), 660, 20);
        flower.scale(100,100);

        rock = new Rock(this.getResources(), 450, 125);
        rock.scale(120,120);

        grass = new Grass(this.getResources(), 250, 110);
        grass.scale(150,150);

        star = new Star(this.getResources(), 0, 0);
        star.scale(50, 50);

        ember = new Ember(this.getResources());

        rockPaths = new RockPaths(this.getResources());

        coin = new Coin(this.getResources());

        road = new Road(this.getResources());

        water = new Water(this.getResources());
    ...
    public void update() {
        ...
        if((moveXDirection != 0 || moveYDirection != 0) && emberXPosition >= 0 && !isDead && !isWin) { // && emberXPosition >= 0
            ...
            Log.d(GameView.VIEW_LOG_TAG, "Move speed: " + (roadLength/secondsToArrival/fps));

            ember.stepFrame();
            coin.stepFrame();

            Log.d(GameView.VIEW_LOG_TAG, "Updating!");
            Log.d(GameView.VIEW_LOG_TAG, "Height diff [" + emberYPosition + ", " + (height-bottomBuffer) + "].");
            Log.d(GameView.VIEW_LOG_TAG, "Width diff [" + emberXPosition + ", " + (roadWidth + (boxSize*numberOfBoxesWidth)/2) + "].");

            if (isFixedWalking) {
                Log.d(GameView.VIEW_LOG_TAG, "Road!");
                moveOnFixedRoad();
            } else if (isWithinGrid(emberXPosition, emberYPosition)) {
                Log.d(GameView.VIEW_LOG_TAG, "Path!");
                moveOnAddedPath();
            } else if(emberXPosition ==  roadWidth + (boxSize*numberOfBoxesWidth)/2 && emberYPosition >= height-bottomBuffer) {
                Log.d(GameView.VIEW_LOG_TAG, "Is win!");
                isWin = true;
            } else {
                Log.d(GameView.VIEW_LOG_TAG, "Is death!");
                isDead = true;
    ...
    private void moveOnFixedRoad() {
        if(moveYDirection == -1 && emberYPosition <= roadHeight/2) {
            Log.d(GameView.VIEW_LOG_TAG, "Moving up, [" + emberXPosition + ", " + emberYPosition + "]");
            moveXDirection = 1;
            moveYDirection = 0;
            emberYPosition = roadHeight/2;
        } else if(moveXDirection == 1 && emberXPosition >= roadWidth + ((numberOfBoxesWidth*boxSize)/2)) {
            Log.d(GameView.VIEW_LOG_TAG, "Moving right, [" + emberXPosition + ", " + emberYPosition + "]");
            moveXDirection = 0;
            moveYDirection = 1;
            emberXPosition = roadWidth + ((numberOfBoxesWidth*boxSize)/2);
        } else if(isWithinGrid(emberXPosition, emberYPosition)) {
            Log.d(GameView.VIEW_LOG_TAG, "Is within grid, [" + emberXPosition + ", " + emberYPosition + "]");
    ...
    private void drawPath(Canvas canvas) {
        ...
        drawRoadPiece(nextType, width-boxSize, 0);
        ...
        for(int i = 0; i < numberOfBoxesWidth; i++) {
            for(int j = 0; j < numberOfBoxesHeight; j++) {
                ...
                if(type > -1)
                    drawRoadPiece(type, getPositionFromBoxNumber(i,roadWidth), getPositionFromBoxNumber(j, roadHeight));
            }
        }
    ...
    private void drawRoadPiece(int type, int x, int y) {
        ...
        roadPaint.setColor(Color.BLACK);
        roadPaint.setStrokeWidth(10);
        int midX = x + boxSize/2;
        int midY = y + boxSize/2;
        switch(type) {
            case 0:
                canvas.drawLine(midX, y, midX, y + boxSize, roadPaint);
                rockPaths.drawImage(type, x, y, canvas);
                break;
            case 1:
                canvas.drawLine(x, midY, x + boxSize, midY, roadPaint);
                rockPaths.drawImage(type, x, y, canvas);
                break;
            case 2:
                canvas.drawLine(midX, y, midX, midY, roadPaint);
                canvas.drawLine(midX, midY, x + boxSize, midY, roadPaint);
                rockPaths.drawImage(type, x, y, canvas);
                break;
            case 3:
                canvas.drawLine(midX, midY, x + boxSize, midY, roadPaint);
                canvas.drawLine(midX, midY, midX, y + boxSize, roadPaint);
                rockPaths.drawImage(type, x, y, canvas);
                break;
            case 4:
                canvas.drawLine(midX, midY, midX, y + boxSize, roadPaint);
                canvas.drawLine(x, midY, midX, midY, roadPaint);
                rockPaths.drawImage(type, x, y, canvas);
                break;
            case 5:
                canvas.drawLine(x, midY, midX, midY, roadPaint);
    ...
    public boolean onTouchEvent(MotionEvent motionEvent) {
        ...
        switch (motionEvent.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_DOWN:
                ...
                Log.i(GameView.VIEW_LOG_TAG, "Touched at [" + motionEvent.getX() + ", " + motionEvent.getY() + "]");
                if(isWithinGrid(motionEvent.getX(), motionEvent.getY()) && !isDead && !isWin) {
                    Log.i(GameView.VIEW_LOG_TAG, "Touched within grid");
                    Point p = getBox((int) motionEvent.getX(), (int) motionEvent.getY());
                    Log.i(GameView.VIEW_LOG_TAG, "Box gotten [" + p.x + ", " + p.y + "]");
                    boolean success = pathPieces.addPathPiece(p.x, p.y, nextType);
                    if(success) {
                        Log.i(GameView.VIEW_LOG_TAG, "Success, change path piece!");
                        nextType = getNextType();
                    }
    ...
    private boolean isWithinGrid(float x, float y) {
        if(x >= roadWidth && x < boxSize*numberOfBoxesWidth + roadWidth &&
            ...
            Log.i(GameView.VIEW_LOG_TAG, "In grid [" + x + ", " + y + "].");
            return true;
        }
        ...
        if(box.x >= 0 && box.x < numberOfBoxesWidth && box.y >= 0 && box.y < numberOfBoxesHeight) {
            Log.i(GameView.VIEW_LOG_TAG, "In box [" + box.x + ", " + box.y + "].");
            return true;
        }*/
        //Log.i(GameView.VIEW_LOG_TAG, "Out of box [" + box.x + ", " + box.y + "].");
        return false;
    }
    ...
    private Point getBox(int in_x, int in_y) {
        ...
        Log.i(GameView.VIEW_LOG_TAG, "Max grid size [" + boxSize*numberOfBoxesWidth + ", " + boxSize*numberOfBoxesHeight + "].");
        Log.i(GameView.VIEW_LOG_TAG, "Relative pos [" + relative_x + ", " + relative_y + "].");
        Log.i(GameView.VIEW_LOG_TAG, "Return box [" + box_x + ", " + box_y + "].");
